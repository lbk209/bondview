...
Every stance calculation produces a final numeric score through either the weighted-sum or rule-mapped path. Direction and strength are derived from that score, while breakdown data explains its calculation without defining a separate scoring path.
