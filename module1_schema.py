...
def validate_module1_config(config: dict) -> dict:
    ...
    def validate_exposure_stance_diagnostics(stance_name, stance):
        diagnostics = stance.get("diagnostics")
        if diagnostics is None:
            return
        if not isinstance(diagnostics, dict):
            add_issue(
        ...
        for field in diagnostics:
            if field != "trace_context":
                add_issue(
                    ...
                    f"diagnostics.{field}",
                    "unknown",
                    "Unknown exposure-stance diagnostics field.",
                )

        trace_context = diagnostics.get("trace_context")
        ...
        if not isinstance(trace_context, dict):
            add_issue(
                ...
                "invalid",
                "diagnostics.trace_context must be a mapping when present.",
            )
            return

        allowed_trace_context_fields = {"group_order", "score_input_order"}
        for field in trace_context:
            if field not in allowed_trace_context_fields:
                add_issue(
                    "exposure_stances",
                    stance_name,
                    f"diagnostics.trace_context.{field}",
                    "unknown",
                    "Unknown diagnostics.trace_context field.",
                )

        group_order = trace_context.get("group_order")
        group_order_field = "diagnostics.trace_context.group_order"
        supported_groups = {
            "raw_inputs",
            "features",
        ...
        if not isinstance(group_order, list):
            add_issue(
                ...
                stance_name,
                group_order_field,
                "missing" if group_order is None else "invalid",
                "diagnostics.trace_context.group_order must be a list containing "
                "raw_inputs, features, and prepared_filtered_inputs exactly once.",
            )
        else:
            seen_groups = set()
            for index, group_name in enumerate(group_order):
                member_field = f"{group_order_field}[{index}]"
                if not non_empty_string(group_name):
                    add_issue(
                        "exposure_stances",
                        stance_name,
                        member_field,
                        "invalid",
                        "diagnostics.trace_context.group_order values must be non-empty strings.",
                    )
                    continue
                if group_name in seen_groups:
                    add_issue(
                        ...
                        member_field,
                        "duplicate",
                        f"diagnostics.trace_context.group_order contains duplicate group: {group_name}.",
                    )
                ...
                if group_name not in supported_groups:
                    add_issue(
                        ...
                        member_field,
                        "unknown",
                        f"Unknown diagnostics.trace_context group: {group_name}.",
                    )
            for missing_group in sorted(supported_groups - seen_groups):
                add_issue(
                    "exposure_stances",
                    stance_name,
                    group_order_field,
                    "missing",
                    f"diagnostics.trace_context.group_order is missing group: {missing_group}.",
                )

        if "score_input_order" not in trace_context:
            return

        score_input_order = trace_context.get("score_input_order")
        score_input_order_field = "diagnostics.trace_context.score_input_order"
        if not isinstance(score_input_order, list) or not score_input_order:
            add_issue(
                "exposure_stances",
                stance_name,
                score_input_order_field,
                "invalid",
                "diagnostics.trace_context.score_input_order must be a non-empty list when present.",
            )
            return
        ...
        if not isinstance(rule_mapped, dict):
            add_issue(
                ...
                score_input_order_field,
                "invalid_scope",
                "diagnostics.trace_context.score_input_order is valid only for a schema-backed rule-mapped stance.",
            )
            return
        ...
        declared_score_inputs = (
            [
                state_input.get("source_score")
                for state_input in state_inputs
                if isinstance(state_input, dict)
                and non_empty_string(state_input.get("source_score"))
            ]
            if isinstance(state_inputs, list)
            else []
        )
        declared_score_input_set = set(declared_score_inputs)
        seen_score_inputs = set()
        for index, score_input in enumerate(score_input_order):
            member_field = f"{score_input_order_field}[{index}]"
            if not non_empty_string(score_input):
                add_issue(
                    "exposure_stances",
                    stance_name,
                    member_field,
                    "invalid",
                    "diagnostics.trace_context.score_input_order values must be non-empty strings.",
                )
                continue
            if score_input in seen_score_inputs:
                add_issue(
                    "exposure_stances",
                    stance_name,
                    member_field,
                    "duplicate",
                    f"diagnostics.trace_context.score_input_order contains duplicate score input: {score_input}.",
                )
            seen_score_inputs.add(score_input)
            if score_input not in declared_score_input_set:
                add_issue(
                    "exposure_stances",
                    stance_name,
                    member_field,
                    "unknown",
                    "diagnostics.trace_context.score_input_order must use source_score values "
                    f"declared by rule_mapped.state_inputs: {score_input}.",
                )
        for missing_score_input in (
            score_input
            for score_input in declared_score_inputs
            if score_input not in seen_score_inputs
        ):
            add_issue(
                "exposure_stances",
                stance_name,
                score_input_order_field,
                "missing",
                "diagnostics.trace_context.score_input_order is missing declared "
                f"source_score: {missing_score_input}.",
            )
    ...
    for stance_name, stance in exposure_stances.items():
        ...
        validate_exposure_stance_diagnostics(stance_name, stance)
